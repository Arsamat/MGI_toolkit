...
log_file <- file.path(LOG_DIR, '260320_iPSC_DE_Analysis.log')
cat('', file = log_file)
log_msg <- function(...) {
...
design <- model.matrix(~ Genotype + Treatment + Genotype:Treatment, data = dge_sub$samples)
...
inter_coefs <- grep('Genotype[^:]+:Treatment|Treatment[^:]+:Genotype', all_coefs, value = TRUE)
log_msg('Genotype terms: ', length(geno_coefs), ' | Treatment terms: ', length(treat_coefs),
        ' | Interaction terms: ', length(inter_coefs))

sample_rows[['GROUP1_iPSC']] <- data.frame(
  group = 'GROUP1', context = 'CellType=iPSC',
  model_design = '~ Genotype + Treatment + Genotype:Treatment',
...
term_map  <- list('Genotype' = geno_coefs, 'Treatment' = treat_coefs, 'Genotype:Treatment' = inter_coefs)
...
for (term_label in names(term_map)) {
  for (coef in term_map[[term_label]]) {
    ...
    n_sig <- count_sig(res)
    log_msg('    Sig: ', n_sig)
    res$model_design <- '~ Genotype + Treatment + Genotype:Treatment'
    res$term <- term_label; res$factor1_value <- fv$factor1_value; res$factor2_value <- fv$factor2_value
    safe_coef <- safe_str(coef)
    write.csv(res, file.path(OUT_G1, paste0('260320_iPSC_DE_GROUP1_iPSC_', safe_coef, '.csv')), row.names = FALSE)
    make_volcano(res, coef, paste0('GROUP1 | iPSC | ', term_label, ' | ', coef, '\n(n sig = ', n_sig, ')'),
                 paste0('260320_iPSC_Volcano_GROUP1_iPSC_', safe_coef, '.png'), OUT_G1)
...
if (length(summary_rows) > 0) {
  write.csv(do.call(rbind, summary_rows),
            file.path(OUT_DE, '260320_iPSC_Summary_GROUP1_DEG_Counts.csv'), row.names = FALSE)
  log_msg('Saved: 260320_iPSC_Summary_GROUP1_DEG_Counts.csv')
}
if (length(sample_rows) > 0) {
  write.csv(do.call(rbind, sample_rows),
            file.path(OUT_DE, '260320_iPSC_Summary_SampleInclusion.csv'), row.names = FALSE)
  log_msg('Saved: 260320_iPSC_Summary_SampleInclusion.csv')
}
