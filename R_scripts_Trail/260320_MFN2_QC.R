...
log_file <- file.path(LOG_DIR, '260320_MFN2_QC.log')
cat('', file = log_file)
log_msg <- function(...) {
...
## ── Load counts ────────────────────────────────────────────────────────────────
COUNTS_FILE <- '/lts/rmlab/rmlab_shared3/alferreiro/BRB_Seq/NRP_FullCounts/MD815_MNP_MFN2HETHOM_Dedup_Counts.txt'
META_FILE   <- '/lts/rmlab/rmlab_shared3/alferreiro/BRB_Seq/NRP_FullCounts/MD815_MNP_MFN2HETHOM_metadata.csv'
...
log_msg('Ensembl version suffixes stripped')
...
saveRDS(dge, file.path(DATA_DIR, '260320_MFN2_DGElist.RDS'))
log_msg('Saved DGEList: data/260320_MFN2_DGElist.RDS')

## ── QC: Overview ──────────────────────────────────────────────────────────────
threshold <- 2e6
p_overview <- ggplot(meta, aes(x = Genotype, y = NumUnqDedupedReads)) +
  ...
  labs(title    = 'Sequencing Effort — MFN2 MNP Rotenone',
       subtitle = 'Dashed line: 2M unique deduped reads; × = below threshold',
       x = 'Genotype', y = 'Unique Deduped Reads') +
  theme_bw() + theme(legend.position = 'bottom')
ggsave(file.path(FIG_DIR, '260320_MFN2_UMIDeDuped_ReadCounts_Overview.png'),
       p_overview, width = 8, height = 5)
log_msg('Saved: qc_figures/260320_MFN2_UMIDeDuped_ReadCounts_Overview.png')
...
p_group <- ggplot(meta, aes(x = Treatment2, y = NumUnqDedupedReads)) +
  ...
  scale_y_continuous(labels = scales::unit_format(unit = 'M', scale = 1e-6)) +
  labs(title = 'Sequencing Effort by Group — MFN2 MNP Rotenone',
       x = 'Treatment', y = 'Unique Deduped Reads') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8),
        legend.position = 'bottom')
ggsave(file.path(FIG_DIR, '260320_MFN2_UMIDeDuped_ReadCounts_ByGroup.png'),
       p_group, width = 9, height = 5)
log_msg('Saved: qc_figures/260320_MFN2_UMIDeDuped_ReadCounts_ByGroup.png')
