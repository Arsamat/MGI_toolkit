...
log_file <- file.path(LOG_DIR, '260320_iPSC_PCA.log')
cat('', file = log_file)
log_msg <- function(...) {
...
dge <- readRDS(file.path(DATA_DIR, '260320_iPSC_DGElist.RDS'))
...
paletteTreatment2 <- c('Vehicle' = '#969696', 'Rotenone_EC25' = '#FDCC8A', 'Rotenone_EC75' = '#D7301F',
                       'Tunicamycin_EC25' = '#BCBDDC', 'Tunicamycin_EC75' = '#54278F')
paletteTimepoint  <- c('6' = '#FEE391', '24' = '#FC4E2A')
shapesGenotype    <- c('WT' = 22, 'LATS2KO' = 23)
shapesTreatment2  <- c('Vehicle' = 22, 'Rotenone_EC25' = 23, 'Rotenone_EC75' = 24,
                       'Tunicamycin_EC25' = 3, 'Tunicamycin_EC75' = 4)
shapesTimepoint   <- c('6' = 21, '24' = 22)
...
save_pdf <- function(p, name) {
  fpath <- file.path(FIG_DIR, paste0('260320_iPSC_PCA_', name, '.pdf'))
...
p1 <- ggplot(pca_dat, aes(PC1, PC2)) +
  ...
  scale_fill_manual(values = paletteTreatment2) + scale_colour_manual(values = paletteTreatment2) +
  scale_shape_manual(values = shapesGenotype) +
  guides(fill = guide_legend(override.aes = list(shape = 21))) + base_layers
save_pdf(p1, 'Genotype_by_Treatment2')
...
p2 <- ggplot(pca_dat, aes(PC1, PC2)) +
  ...
  guides(fill = guide_legend(override.aes = list(shape = 21))) + base_layers
save_pdf(p2, 'Treatment2_by_Genotype')
...
p4 <- ggplot(pca_dat, aes(PC1, PC2)) +
  geom_point(aes(fill = Treatment, colour = Treatment, shape = Timepoint), size = 4, alpha = 0.8) +
  ...
  guides(fill = guide_legend(override.aes = list(shape = 21))) + base_layers
save_pdf(p4, 'Timepoint_by_Treatment')

p5 <- ggplot(pca_dat %>% arrange(desc(NumUnqDedupedReads)) %>% mutate(Low = NumUnqDedupedReads < 2e6),
             aes(PC1, PC2)) +
  ...
  base_layers
save_pdf(p5, 'NumUnqDedupedReads')
