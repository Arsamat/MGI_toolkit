...
log_file <- file.path(LOG_DIR, '260320_WT_MNP_PCA.log')
cat('', file = log_file)
log_msg <- function(...) {
...
shapesTreatment  <- c('Vehicle' = 22, 'Rotenone' = 24)
shapesTimepoint  <- c('6' = 21, '12' = 22, '24' = 23, '48' = 24)
...
save_pdf <- function(p, name) {
  fpath <- file.path(FIG_DIR, paste0('260320_WT_MNP_PCA_', name, '.pdf'))
...
p1 <- ggplot(pca_dat, aes(PC1, PC2)) +
  geom_point(aes(fill = Timepoint, colour = Timepoint, shape = Treatment), size = 4, alpha = 0.8) +
...
save_pdf(p1, 'Treatment_by_Timepoint')
...
p2 <- ggplot(pca_dat, aes(PC1, PC2)) +
  ...
  guides(fill = guide_legend(override.aes = list(shape = 21))) + base_layers
save_pdf(p2, 'Timepoint_by_Treatment')

p3 <- ggplot(pca_dat %>% arrange(desc(NumUnqDedupedReads)) %>% mutate(Low = NumUnqDedupedReads < 2e6),
             aes(PC1, PC2)) +
  ...
  base_layers
save_pdf(p3, 'NumUnqDedupedReads')
