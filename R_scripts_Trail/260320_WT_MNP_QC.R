...
log_file <- file.path(LOG_DIR, '260320_WT_MNP_QC.log')
cat('', file = log_file)
log_msg <- function(...) {
...
## ── Load counts ────────────────────────────────────────────────────────────────
COUNTS_FILE <- '/lts/rmlab/rmlab_shared3/alferreiro/BRB_Seq/NRP_FullCounts/MD867_MNPs_DedupCounts.txt'
META_FILE   <- '/lts/rmlab/rmlab_shared3/alferreiro/BRB_Seq/NRP_FullCounts/MD867_MNPs_metadata.csv'
...
## ── Factor levels ──────────────────────────────────────────────────────────────
meta <- meta %>% mutate(
  Treatment = factor(Treatment, levels = c('Vehicle', 'Rotenone')),
  Timepoint = factor(as.character(Timepoint), levels = c('6', '12', '24', '48')),
  Run       = factor(Run)
)

## ── Color palettes ─────────────────────────────────────────────────────────────
paletteTreatment <- c('Vehicle' = '#969696', 'Rotenone' = '#D7301F')
paletteTimepoint <- c('6' = '#FEE391', '12' = '#FEB24C', '24' = '#FC4E2A', '48' = '#800026')
...
saveRDS(dge, file.path(DATA_DIR, '260320_WT_MNP_DGElist.RDS'))
log_msg('Saved DGEList: data/260320_WT_MNP_DGElist.RDS')
...
p_overview <- ggplot(meta, aes(x = Timepoint, y = NumUnqDedupedReads)) +
  ...
  labs(title    = 'Sequencing Effort — WT MNP Rotenone Timecourse',
       subtitle = 'Dashed line: 2M unique deduped reads; × = below threshold',
       x = 'Timepoint (hr)', y = 'Unique Deduped Reads') +
  theme_bw() + theme(legend.position = 'bottom')
ggsave(file.path(FIG_DIR, '260320_WT_MNP_UMIDeDuped_ReadCounts_Overview.png'),
       p_overview, width = 8, height = 5)
log_msg('Saved: qc_figures/260320_WT_MNP_UMIDeDuped_ReadCounts_Overview.png')
...
p_group <- ggplot(meta, aes(x = Treatment, y = NumUnqDedupedReads)) +
  ...
  scale_y_continuous(labels = scales::unit_format(unit = 'M', scale = 1e-6)) +
  labs(title = 'Sequencing Effort by Timepoint — WT MNP Rotenone Timecourse',
       x = 'Treatment', y = 'Unique Deduped Reads') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8),
        legend.position = 'bottom')
ggsave(file.path(FIG_DIR, '260320_WT_MNP_UMIDeDuped_ReadCounts_ByGroup.png'),
       p_group, width = 9, height = 5)
log_msg('Saved: qc_figures/260320_WT_MNP_UMIDeDuped_ReadCounts_ByGroup.png')
