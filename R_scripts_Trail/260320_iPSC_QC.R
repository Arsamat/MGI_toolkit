...
ANA_DIR  <- file.path(BASE_DIR, '260320_iPSC_WT_LATS2KO_Rot_Tun_Timecourse_Analyses')
...
log_file <- file.path(LOG_DIR, '260320_iPSC_QC.log')
cat('', file = log_file)
log_msg <- function(...) {
...
## ── Load counts ────────────────────────────────────────────────────────────────
COUNTS_FILE <- '/lts/rmlab/rmlab_shared3/alferreiro/BRB_Seq/NRP_FullCounts/MD893_Treated_iPSCs_DedupCounts.txt'
META_FILE   <- '/lts/rmlab/rmlab_shared3/alferreiro/BRB_Seq/NRP_FullCounts/MD893_Treated_iPSCs_metadata.csv'
...
meta <- meta %>% mutate(
  ...
  Treatment  = factor(Treatment,  levels = c('Vehicle', 'Rotenone', 'Tunicamycin')),
  Treatment2 = factor(Treatment2, levels = c('Vehicle', 'Rotenone_EC25', 'Rotenone_EC75',
                                              'Tunicamycin_EC25', 'Tunicamycin_EC75')),
  Timepoint  = factor(as.character(Timepoint), levels = c('6', '24')),
  Run        = factor(Run)
)

## ── Color palettes ─────────────────────────────────────────────────────────────
paletteGenotype   <- c('WT' = '#555555', 'LATS2KO' = '#3182BD')
paletteTreatment  <- c('Vehicle' = '#969696', 'Rotenone' = '#FC8D59', 'Tunicamycin' = '#9E9AC8')
paletteTreatment2 <- c(
  'Vehicle'          = '#969696',
  'Rotenone_EC25'    = '#FDCC8A',
  'Rotenone_EC75'    = '#D7301F',
  'Tunicamycin_EC25' = '#BCBDDC',
  'Tunicamycin_EC75' = '#54278F'
)
paletteTimepoint <- c('6' = '#FEE391', '24' = '#FC4E2A')
...
saveRDS(dge, file.path(DATA_DIR, '260320_iPSC_DGElist.RDS'))
log_msg('Saved DGEList: data/260320_iPSC_DGElist.RDS')
...
p_overview <- ggplot(meta, aes(x = Timepoint, y = NumUnqDedupedReads)) +
  ...
  geom_jitter(aes(colour = Treatment2,
                  shape  = NumUnqDedupedReads < threshold),
              width = 0.2, size = 2.5, alpha = 0.85) +
  facet_wrap(~ Genotype, nrow = 1) +
  scale_colour_manual(values = paletteTreatment2, name = 'Treatment') +
  scale_shape_manual(values = c('TRUE' = 4, 'FALSE' = 16),
                     name = 'Below 2M', labels = c('TRUE' = 'Yes', 'FALSE' = 'No')) +
  scale_y_continuous(labels = scales::unit_format(unit = 'M', scale = 1e-6)) +
  labs(title    = 'Sequencing Effort — iPSC LATS2KO Rotenone/Tunicamycin Timecourse',
       subtitle = 'Dashed line: 2M unique deduped reads; × = below threshold',
       x = 'Timepoint (hr)', y = 'Unique Deduped Reads') +
  theme_bw() + theme(legend.position = 'bottom')
ggsave(file.path(FIG_DIR, '260320_iPSC_UMIDeDuped_ReadCounts_Overview.png'),
       p_overview, width = 10, height = 5)
log_msg('Saved: qc_figures/260320_iPSC_UMIDeDuped_ReadCounts_Overview.png')

## ── QC: By group ──────────────────────────────────────────────────────────────
p_group <- ggplot(meta, aes(x = Treatment2, y = NumUnqDedupedReads)) +
  geom_hline(yintercept = threshold, linetype = 'dashed', colour = 'darkred', linewidth = 0.8) +
  ...
  scale_y_continuous(labels = scales::unit_format(unit = 'M', scale = 1e-6)) +
  labs(title = 'Sequencing Effort by Group — iPSC LATS2KO Timecourse',
       x = 'Treatment', y = 'Unique Deduped Reads') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7),
        strip.text  = element_text(size = 8), legend.position = 'bottom')
ggsave(file.path(FIG_DIR, '260320_iPSC_UMIDeDuped_ReadCounts_ByGroup.png'),
       p_group, width = 10, height = 6)
log_msg('Saved: qc_figures/260320_iPSC_UMIDeDuped_ReadCounts_ByGroup.png')
